<script lang="ts">
  ...
  async function handleSubmit() {
    ...
    try {
      const response = await fetch(`${apiBaseUrl}/upload/create`, {
        method: "POST",
        headers: {
      ...
      console.log(`${apiBaseUrl}/upload/create`);

      if (response.ok) {
        console.log("Successful create entry. Status: ", response.status);
...
<div class="h-screen w-full bg-slate-950">
  <SectionWrapper>
    <h2 class="text-2xl font-bold mb-6 text-gray-400">Create</h2>

    <form on:submit|preventDefault={handleSubmit} class="w-[800px]">
      <!-- Post ID -->
      <div class="mb-4">
        <Label class="text-gray-400 font-bold">Post ID</Label>
        <Input type="number" id="postId" bind:value={postId} required />
      </div>
